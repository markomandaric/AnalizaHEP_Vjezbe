...
void Analyzer::Plot(TString Path)
{
	...
	*Z1 = *l1 + *l2;
        ...
        *higgs = *Z1 + *Z2;
        //h4->Fill(higgs->M(),w);
	if(Path=="ggH125")
		signal->Fill(higgs->M(),w);
	else if(Path=="qqZZ")
		background->Fill(higgs->M(),w);
}
...
gStyle->SetOptStat(0);
	...
	canvas->SaveAs("zdk2_"+Path+".pdf");
	canvas->SaveAs("zdk2"+Path+".png");
	canvas->SaveAs("zdk2"+Path+".root");
	...
	canvas2->SaveAs("Higgsw"+Path+".pdf");
	canvas2->SaveAs("Higgsw"+Path+".png");
	canvas2->SaveAs("Higgsw"+Path+".root");
}
